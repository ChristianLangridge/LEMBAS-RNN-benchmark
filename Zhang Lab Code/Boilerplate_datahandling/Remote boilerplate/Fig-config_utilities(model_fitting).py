...
print(type(mlr_y_pred_train), mlr_y_pred_train.shape)
...
print(type(xgbrf_y_pred_train), xgbrf_y_pred_train.shape)
...
RNN_loaded = load_model_from_checkpoint(
                ...
                X_in_df=pd.DataFrame(x_train),  # passing as df not tensors
                y_out_df=pd.DataFrame(y_train),  # passing as df not tensors
                device='cpu',
...
with torch.no_grad():  # Disable gradients for inference
    ...
    rnn_y_pred_train = rnn_y_pred.detach().numpy()
print(type(rnn_y_pred_train), rnn_y_pred_train.shape)
...
predictions_train = {
    "MLR": mlr_y_pred_train,
    "XGBRFRegressor": xgbrf_y_pred_train,
    "RNN": rnn_y_pred_train
}

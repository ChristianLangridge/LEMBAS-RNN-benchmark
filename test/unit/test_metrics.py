...
class TestComputeMetricsPerGene:
    ...
    def test_perfect_per_gene_predictions(self, y_true_df, perfect_y_pred):
        """Perfect predictions must give r2=1, pearson_r=1 for every gene."""
        result = compute_metrics_per_gene(y_true_df, perfect_y_pred)
        assert (result["r2"] == pytest.approx(1.0, abs=1e-5)).all()
        assert (result["pearson_r"] == pytest.approx(1.0, abs=1e-5)).all()
